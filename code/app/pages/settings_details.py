...
dash.register_page(__name__, path_template="/settings/<uid>", name="Settings Details")
...
def render_config_row(uid: str, idx: int = 0,
                      signal_asset_id: str = "",
    ...
    return AlphaRow([
        ...
        dbc.Col(dbc.Input(value=platform_asset_id, id={"type": "platform-asset", "index": idx}), width=PLATFORM_ID_WIDTH),
        dbc.Col(select_entry, width=ENTRY_STAGGER_WIDTH),
        dbc.Col(select_size, width=SIZE_STAGGER_WIDTH),
        dbc.Col(dbc.Input(type="number", value=n_staggers, id={"type": "n-staggers", "index": idx}, min=1), width=N_STAGGERS_WIDTH),
        dbc.Col(dbc.Input(type="number", value=size, id={"type": "size", "index": idx}, step=0.01, min=0.01), width=SIZE_WIDTH),
        dbc.Col(dbc.Input(type="number", value=decimal_points, id={"type": "decimals", "index": idx}, min=0, step=1), width=DECIMALS_WIDTH),
        dbc.Col(dbc.Button("🗑️", id={"type": "delete-config", "index": idx}, color="danger", size="sm"), width=DELETE_WIDTH),
...
def extract_uid_from_url(pathname: str):
    ...
    return uid, PageHeader(f"Trade Settings for {uid}")
...
def handle_config_modifications(
    add_clicks, delete_clicks, signal_ids, platform_ids,
    entry_staggers, size_staggers, n_staggers, sizes, decimals, uid
):
    triggered = ctx.triggered_id
    rows = list(zip(
        signal_ids, platform_ids, entry_staggers, size_staggers,
        n_staggers, sizes, decimals
    ))
    ...
    return [render_header_row()] + [render_config_row(uid, idx=i,
        signal_asset_id=r[0], platform_asset_id=r[1], entry_stagger=r[2],
        size_stagger=r[3], n_staggers=r[4], size=r[5], decimal_points=r[6]
...
def save_settings(_, uid, signal_ids, platform_ids, entry_staggers, size_staggers, n_staggers, sizes, decimals):
    submitted = [
        ...
        for s, p, e, ss, n, sz, d in zip(signal_ids, platform_ids, entry_staggers, size_staggers, n_staggers, sizes, decimals)
        if s and p
    ]
