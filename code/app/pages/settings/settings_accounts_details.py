...
def _config_modal_fields(
    ...
    return html.Div([
        dbc.Input(id=f"{prefix}-signal-asset-id", value=signal_asset_id, placeholder="Signal Asset ID",
                  className="mb-2"),
        dbc.Input(id=f"{prefix}-platform-asset-id", value=platform_asset_id, placeholder="Platform Asset ID",
                  className="mb-2"),
        dbc.Select(
        ...
        dbc.Input(id=f"{prefix}-n-staggers", type="number", value=n_staggers, min=1, className="mb-2",
                  placeholder="Staggers"),
        dbc.Input(id=f"{prefix}-risk", type="number", value=risk_percent, step=0.01, min=0.0, className="mb-2",
                  placeholder="Risk %"),
        dbc.Input(id=f"{prefix}-lot-size", type="number", value=lot_size, step=0.01, min=0.0, className="mb-2",
                  placeholder="Lot Size"),
        dbc.Input(id=f"{prefix}-decimals", type="number", value=decimals, step=1, min=0, className="mb-2",
...
def _action_buttons(signal_asset_id: str):
    return AlphaRow([
        AlphaCol(AlphaButton("✏️", {"type": "edit-config", "index": signal_asset_id},
                             style={"backgroundColor": "#FFC107", "width": "40px"}).render(), width="auto"),
        AlphaCol(AlphaButton("🗑️", {"type": "delete-config", "index": signal_asset_id},
...
def _enabled_button(account_id: str, signal_asset_id: str, is_enabled: bool):
    ...
    return AlphaButton(label, {
        "type": "toggle-enabled",
        "account": account_id,
        "index": signal_asset_id,
    }, style={"backgroundColor": color, "width": "40px"}).render()
...
def build_table(account_id: str) -> html.Table:
    ...
    headers = [
        "Signal Asset ID", "Platform Asset ID", "Entry Stagger",
        "# Staggers", "Risk %", "Lot Size", "Decimals", "Enabled", "Actions"
    ]
    rows = [
        [
            c.signal_asset_id,
        ...
        return PageBody([
            ...
            MainContent([
                ...
                AlphaRow([
                    ...
                    AlphaCol(AlphaButton("🧨 Delete All Configs", "delete-all-configs-btn",
                                         style={"backgroundColor": "#dc3545"}).render(), width="auto"),
                ]),
                AlphaModal("add-config-modal", "Add New Config", _config_modal_fields("modal-add"),
                           "confirm-add-config", "cancel-add-config").render(),
                AlphaModal("edit-config-modal", "Edit Config", _config_modal_fields("modal-edit"),
                           "confirm-edit-config", "cancel-edit-config").render(),
            ])
        ])
...
def render_table(uid): return build_table(uid)
...
def open_edit_modal(n_clicks_list, account_id):
    ...
    config = next(
        (c for c in get_configs_by_account_id(account_id) if c.signal_asset_id == triggered["index"]),
        None
    )
    if not config:
        raise dash.exceptions.PreventUpdate
