...
def _settings_modal_fields(prefix: str, key="", value=""):
    return html.Div([
        dbc.Input(id=f"{prefix}-key", value=key, placeholder="Setting Key", className="mb-2", disabled=(prefix == "modal-edit")),
...
def build_table():
    ...
    for setting in settings:
        actions = AlphaRow([
            AlphaCol(AlphaButton("✏️", {"type": "edit-setting", "index": setting.key}, style={"backgroundColor": "#FFC107", "width": "40px"}).render(), width="auto")
        ])

        if setting.key not in REQUIRED_KEYS:
            actions.children.append(
                AlphaCol(AlphaButton("🗑️", {"type": "delete-setting", "index": setting.key}, style={"backgroundColor": "#DC3545", "width": "40px"}).render(), width="auto")
            )
