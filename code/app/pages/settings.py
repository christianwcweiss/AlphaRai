...
from services.credential_settings import (
    get_all_credential_settings,
    upsert_credential_settings,
    delete_credential_settings,
)
...
def render_account_row(platform: Platform, label: str = "", secret_name: str = "", idx: int = 0, uid: Optional[str] = None) -> AlphaRow:
    ...
    settings_link = dcc.Link("⚙️", href=f"/settings/{uid}")
...
class SettingsPage(BasePage):
    def render(self):
        return PageBody([
            ...
            PageHeader("Settings"),
            MainContent([
                html.H4("MetaTrader Accounts", className="mt-4"),
                ...
                html.H4("IG Accounts", className="mt-4"),
...
def load_mt5_credentials_on_page_load(_):
    credentials = [c for c in get_all_credential_settings() if Platform(c.platform.upper()) is Platform.METATRADER]
    return [render_account_row(Platform.METATRADER, c.friendly_name, c.secret_name, idx=i, uid=c.uid) for i, c in enumerate(credentials)]
...
def load_ig_credentials_on_page_load(_):
    credentials = [c for c in get_all_credential_settings() if Platform(c.platform.upper()) is Platform.IG]
    return [render_account_row(Platform.IG, c.friendly_name, c.secret_name, idx=i, uid=c.uid) for i, c in enumerate(credentials)]
...
def save_all_credentials(_, mt5_labels, mt5_secrets, mt5_uids, ig_labels, ig_secrets, ig_uids):
    mt5_submitted = [dict(label=l, secret=s, uid=u) for l, s, u in zip(mt5_labels, mt5_secrets, mt5_uids) if l and s and u]
    ig_submitted = [dict(label=l, secret=s, uid=u) for l, s, u in zip(ig_labels, ig_secrets, ig_uids) if l and s and u]

    all_existing = get_all_credential_settings()
    ...
    for row in mt5_submitted:
        upsert_credential_settings(Platform.METATRADER.value, row["label"], row["secret"], row["uid"])
    for old_uid in mt5_existing_uids - {r["uid"] for r in mt5_submitted}:
        delete_credential_settings(Platform.METATRADER.value, old_uid)

    for row in ig_submitted:
        upsert_credential_settings(Platform.IG.value, row["label"], row["secret"], row["uid"])
    for old_uid in ig_existing_uids - {r["uid"] for r in ig_submitted}:
        delete_credential_settings(Platform.IG.value, old_uid)
