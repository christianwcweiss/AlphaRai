from sqlalchemy import Boolean, Column, Enum, Float, Integer, String, PrimaryKeyConstraint
...
class AccountConfig(Base):  # type: ignore  # pylint: disable=too-few-public-methods
    ...
    __table_args__ = (
        PrimaryKeyConstraint("account_id", "platform_asset_id", name="pk_account_asset"),
    )
    ...
    entry_stagger_method = Column(String, default=StaggerMethod.FIBONACCI.name, nullable=False)
    ...
    account = relationship(
        "Account",
        back_populates="account_configs"
    )
