...
class AccountConfigService:
    ...
    def get_configs_by_account(account_uid: str) -> List[AccountConfig]:
        ...
        with MainSessionLocal() as session:
            account = (
                session.query(Account).options(joinedload(Account.account_configs)).filter_by(uid=account_uid).first()
            )
            if account:
    ...
    def sync_with_mt5(account_uid: str, secret_id: str) -> None:
        ...
        AccountConfigService.delete_all_configs_for_account(account_uid)
        ...
        configs = [
            {
                ...
                "entry_stagger_method": StaggerMethod.FIBONACCI.name,
                "entry_offset": 0.0,
                "n_staggers": 3,
                "risk_percent": 0.5,
