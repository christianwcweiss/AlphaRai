...
def _sync_trades_into_db(days: int) -> Dict[str, int]:
    results = []
...
def get_all_groupby_combinations() -> List[Tuple[bool, ...]]:
    return [tuple(i in combination for i in range(5)) for r in range(6) for combination in combinations(range(5), r)]
...
def sync_trades_from_all_accounts(days: int = 9999) -> str:
    ...
    results = _sync_trades_into_db(days)

    group_by_combinations = get_all_groupby_combinations()
