...
class TradeMetricOverTime(ABC):
    ...
    def _normalize_time(self, data_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    def apply_rolling(self, df: pd.DataFrame, value_cols: List[str]) -> pd.DataFrame:
        """Apply rolling mean to the specified columns."""
        df = df.copy()
    ...
    def calculate_ungrouped(self, data_frame: pd.DataFrame) -> pd.DataFrame:
        """Calculate the metric without grouping by account_id."""
    ...
    def get_rolling_windows(
        self, df: pd.DataFrame, skip_head: bool = False, aggregation_resolution: str = "D"
